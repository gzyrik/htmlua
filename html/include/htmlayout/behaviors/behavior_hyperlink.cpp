...
struct hyperlink: public behavior
{
    ...
    void notify(dom::element& el, NMHL_HYPERLINK::type code)
    {
      // send notification
        ...
        const wchar_t *pszTarget = el.attribute("-target", (const wchar_t *)0);
        if(pszTarget)
        {
    ...
    bool try_to_load( dom::element& root, const wchar_t* url, const wchar_t* target )
    {
      ...
      //dom::element target_el = root.find_first("#%S",target);   
        dom::element target_el = root.find_first("[name=\"%S\"]", target);
      if(!target_el.is_valid())
        return false;
      {
          //try web behavior
          XCALL_PARAMS prm("navigate");
          json::value argv(url);
          prm.argv = &argv;
          prm.argc = 1;
          if(target_el.call_behavior_method(&prm))
              return true;
      }
