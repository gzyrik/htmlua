...
namespace aux 
{
  ...
  public:
    explicit w2a(const wchar_t* wstr, unsigned int nu=-1):buffer(0),n(0)
      {
      if(wstr){
        if (nu == -1) nu= (unsigned int)wcslen(wstr);
        init(wstr, nu);
      }
    }
    ...
    explicit w2a(const slice<wchar_t>& s);
  ...
  public:
    explicit a2w(const char* str, unsigned int n=-1):buffer(0), nu(0)
      {
      if(str){
        if (n == -1) n= (unsigned int)strlen(str);
        init(str, n);
      }
    }
    explicit a2w(const slice<char>& s);
  ...
  public:
      ...
      unsigned int length() const { return a_len; }
  };
  class a2utf
  {
      pod::byte_buffer ubuf;
      const byte* utf_str;
      unsigned int utf_len;
  public:
      explicit a2utf(const char* str, unsigned int n=-1)
      {
          pod::wchar_buffer wbuf;
      ...
      operator const byte*() { return utf_str; }
      operator const char*() { return (const char*)utf_str; }
      unsigned int length() const { return utf_len; }
  };
  ...
  public:
    explicit w2utf(const wchar_t* wstr, size_t nu=-1)
    { 
      if(wstr)
      {
        if (nu == -1) nu = wcslen(wstr);
